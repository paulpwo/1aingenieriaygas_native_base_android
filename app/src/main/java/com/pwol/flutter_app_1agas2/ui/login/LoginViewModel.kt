...
import com.pwol.flutter_app_1agas2.data.Result
...
class LoginViewModel(private val loginRepository: LoginRepository) : ViewModel() {
    ...
    fun login(username: String, password: String, context: Context) {
        ...
        if (result is Result.Success) {
            _loginResult.value =
                LoginResult(success = LoggedInUserView(displayName = result.data.displayName, email = result.data.email, token = result.data.token, image = result.data.image))
        } else {
